...
async function checkStatusOfPackageUpload({ productId, operationId, onRateLimit }) {
...
function uploadZip({ zip, productId, onRateLimit }) {
...
function publishSubmission({ productId, devChangelog, onRateLimit }) {
    return requestWithRetry({
...
async function checkPublishStatus({ productId, operationId, onRateLimit }) {
...
export async function deployToEdgePublishApi({ productId, clientId, apiKey, zip, devChangelog }, { logger, isVerbose, setStatus, setZipPath } = {}) {
    ...
    try {
        ...
        await (logger?.countdown?.(inProgressRetryIntervalMs / 1000, remaining => `A submission is already in progress. Retrying in ${remaining}s`) ?? setTimeout(inProgressRetryIntervalMs));
        await publishAndVerify();
    }
