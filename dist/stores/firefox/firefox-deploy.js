...
import { z } from "zod";
let httpClient;
function authHeader(jwtIssuer, jwtSecret) {
...
function uploadZip({ zip, jwtIssuer, jwtSecret, onRateLimit }) {
...
async function createNewVersion({ slug, uuid, changelog, changelogLang, devChangelog, zip, jwtIssuer, jwtSecret, logger, onRateLimit }) {
    ...
    return requestWithRetry({
        sendRequest: () => httpClient.post(`addon/${slug}/versions/`, JSON.stringify({
            upload: uuid,
...
async function validateUpload({ uuid, jwtIssuer, jwtSecret, onRateLimit }) {
...
function uploadSourceCodeIfNeeded({ slug, zipSource, version, jwtIssuer, jwtSecret, onRateLimit }) {
    const formData = buildFormData([
...
export async function deployToFirefox({ extId, jwtIssuer, jwtSecret, zip, zipSource = "", changelog = "", changelogLang, devChangelog = "" }, { logger, isVerbose, setStatus, setZipPath } = {}) {
