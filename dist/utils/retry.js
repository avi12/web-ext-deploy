...
function getBackoffDelayMs(attempt, maxMs = 5_000) {
...
export function createRateLimitHandler({ manualDeployUrl, formatError, logger, maxWaitSeconds = 60, getWaitSeconds = () => 60 }) {
    return (async (response) => {
        ...
        if (logger?.countdown) {
            await logger.countdown(secondsToWait, remaining => `Too many requests. Retrying in ${remaining}s\nOr, you can deploy manually: ${manualDeployUrl}`);
        }
        else {
...
function extractApiMessage(data, statusText) {
...
export async function requestWithRetry({ sendRequest, parseResponse, formatError, errorContext, onRateLimit, maxRetries = DEFAULT_MAX_RETRIES, maxBackoffMs = 5_000 }) {
