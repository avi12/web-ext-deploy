...
import { createInkLogger, createPreDeployUI } from "./ui/ink-logger.js";
import { green } from "./ui/logging.js";
import { toError } from "./utils/retry.js";
import { z } from "zod";
async function runStoreDeploy(store, json, inkLogger, isDryRun, isVerbose, isAutoFetchCookies, mode) {
...
export async function runDeploy(argv) {
    ...
    const results = await Promise.all(storeEntries.map(([store, json]) => runStoreDeploy(store, json, inkLogger, isDryRun, isVerbose, isAutoFetchCookies, mode)));
    const failures = [];
    const helpTables = [];
