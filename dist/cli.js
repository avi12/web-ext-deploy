...
import { getZodBaseType, getZodDescription, isZodOptional, unwrapZod, zodObjectEntries } from "./utils/zod.js";
import yargs from "yargs";
function schemaToOptions(store, schema, demandRequired = false) {
    const options = {};
...
function buildEnvModeCliOnlyOptions(store, allOptions) {
    ...
    return Object.fromEntries(Object.entries(allOptions)
        .filter(([key]) => cliOverridableInEnvMode.has(key))
        .map(([key, option]) => [key, { ...option, description: (option.description ?? "").replace(" [required]", "") }]));
}
let allStoreOptions = {};
const storeOptionGroups = {};
let envModeCliOnlyOptions = {};
for (const store of storeRegistry) {
    const storeOptions = schemaToOptions(store.name, store.schema);
    allStoreOptions = { ...allStoreOptions, ...storeOptions };
    storeOptionGroups[store.name] = Object.keys(storeOptions).filter((key) => storeRegistry.some(({ name }) => key.startsWith(`${name}-`)));
...
function applyStoreGroups(builder, groups = storeOptionGroups) {
...
function stripCamelCaseArgs(message) {
    return message.replace(/Unknown arguments?: (.+)/, (_, args) => {
...
export const parser = yargs(process.argv.slice(2))
    ...
    .command("env", "Read from .env files", builder => {
    ...
    .command("cli", "Pass arguments directly", builder => {
    ...
    .command("chrome-token", "Get a Chrome Web Store refresh token", builder => builder.version(false).options(schemaToOptions("base", ChromeTokenOptionsSchema, true)), handleChromeToken)
    ...
    .exitProcess(false)
    .help();
async function handleCommand(run) {
    try {
        await run();
    }
    catch (error) {
...
function handleChromeToken(argv) {
