...
export function prepareFirefoxOptions(options: FirefoxOptionsSubmissionApi) {
  ...
  if (correctedOptions.zipSource && !getIsFileExists(correctedOptions.zipSource)) {
    throw new Error(storeError(`Zip source doesn't exist: ${getFullPath(correctedOptions.zipSource)}`));
  }
