...
function tryDeployStore(options: unknown, store: string): Promise<boolean> {
  try {
    return deployStore(options, store, undefined, undefined, true);
  } catch (error) {
...
describe("dry-run mode", () => {
  describe("single store validation", () => {
    for (const [store,
      ...
      it(`validates ${store} options without deploying`, async () => {
        const result = await deployStore(options, store, undefined, undefined, true);
        expect(result).toBe(true);
      });
    }

    it("throws for unknown store", () => {
      expect(() => deployStore({}, "unknown-store", undefined, undefined, true)).toThrow("Unknown store");
    });
  });
  ...
  describe("multi-store validation", () => {
    ...
    it("validates chrome + firefox together", async () => {
      const [chrome,
        ...
        deployStore(validInputs.chrome, "chrome", undefined, undefined, true),
        deployStore(validInputs.firefox, "firefox", undefined, undefined, true)
      ]);
    ...
    it("validates edge + opera together", async () => {
      const [edge,
        ...
        deployStore(validInputs.edge, "edge", undefined, undefined, true),
        deployStore(validInputs.opera, "opera", undefined, undefined, true)
      ]);
  ...
  describe("partial config errors", () => {
    it("rejects chrome with only extId (missing refreshToken, zip)", () => {
      expect(() => deployStore({"extId": "abc123"}, "chrome", undefined, undefined, true)).toThrow();
    });
  ...
  describe("missing required fields with empty strings", () => {
    it("rejects chrome with empty extId", () => {
      expect(() => deployStore({...validInputs.chrome,
        "extId": ""}, "chrome", undefined, undefined, true)).toThrow(
        /No extension ID/
      );
    });
    ...
    it("rejects chrome with empty refreshToken", () => {
      expect(() =>
        deployStore({...validInputs.chrome,
          "refreshToken": ""}, "chrome", undefined, undefined, true)
    ...
    it("rejects firefox with empty jwtIssuer", () => {
      expect(() =>
        deployStore({...validInputs.firefox,
          "jwtIssuer": ""}, "firefox", undefined, undefined, true)
    ...
    it("rejects opera with empty sessionid", () => {
      expect(() =>
        deployStore({...validInputs.opera,
          "sessionid": ""}, "opera", undefined, undefined, true)
    ...
    it("rejects non-existent zip path", () => {
      expect(() =>
        deployStore({...validInputs.chrome,
          "zip": "nonexistent.zip"}, "chrome", undefined, undefined, true)
